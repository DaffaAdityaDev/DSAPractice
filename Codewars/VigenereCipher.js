...
function VigenèreCipher(key, abc) {
    this.encode = function (str) {
        ...
        for (let i = 0; i < str.length; i++) {
            let row = str[i]
            let col = key[i % key.length];
            if (abc.indexOf(row) >= 0) {
                cipherText += abc[(abc.indexOf(row) + abc.indexOf(col)) % abc.length];
            } else {
                cipherText += row;
            }
        }
    ...
    this.decode = function (str) {
        ...
        for (let i = 0; i < str.length; i++) {
            ...
            let col = key[i % key.length];
            if (abc.indexOf(row) >= 0) {
                deCipherText += abc[(abc.indexOf(row) + abc.length - abc.indexOf(col)) % abc.length];
            } else {
                deCipherText += row;
            }
        }
